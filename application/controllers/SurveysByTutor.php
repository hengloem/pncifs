...
class SurveysByTutor extends CI_Controller {
    /**
    * Default constructor
    */
    ...
    public function index() {
        ...
        if($id == NULL){
            $index = $this->Tutorsurvey_model->m_get_student_belong_to_a_tutor();
            foreach ($index as $index_data){
                $id = $index[0]['Id'];
            }
            $data['student_survey_data'] = $this->Tutorsurvey_model->m_get_survey_belong_to_a_student($id);
            
            //get checked survey that tutor has completed
            $data['tutor_checked_survey_data'] = $this->Tutorsurvey_model->m_get_checked_survey($id);
        }else{
            $data['student_survey_data'] = $this->Tutorsurvey_model->m_get_survey_belong_to_a_student($id); 
        ...
        $this->load->view('tutor_surveys/index', $data);
    ...
    public function c_preview_survey_detail($id, $userId){
         ...
         */
        $data['surveys_data'] = $this->Tutorsurvey_model->m_get_survey_preview($id, $userId);
        //$data['questions_data'] = $this->Tutorsurvey_model->m_get_question_preview($id, $userId);
        $data['answers_data'] = $this->Tutorsurvey_model->m_get_answer_preview($id, $userId);
        
        //to get the feature of the page
        $data['title'] = 'Preview survey information';
        $this->load->view('templates/header', $data);
        $this->load->view('templates/menu', $data);
        $this->load->view('tutor_surveys/detail', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function c_process_report($id){
        ...
        $insert_report =   $this->Tutorsurvey_model->m_insert_report($id, $reportsText);
        if ($insert_report > 0) {
    ...
    public function c_process_submit($id){
        $data = getUserContextData($this);
        $push_submit = $data['tutor_checked_survey_data'] = $this->Tutorsurvey_model->m_update_to_checked_survey($id);
        if($push_submit){
    ...
    public function c_preview_checked_survey($id, $userId){
         ...
         * get specific survey, question ; aswer that belong to a specific student also
         */
        $data['checked_surveys_data'] = $this->Tutorsurvey_model->m_checked_survey($id, $userId);
        $data['checked_questions_data'] = $this->Tutorsurvey_model->m_get_question_preview($id, $userId);
        $data['checked_answers_data'] = $this->Tutorsurvey_model->m_get_answer_preview($id, $userId);
        $data['checked_report_data'] = $this->Tutorsurvey_model->m_get_checked_report($id, $userId);
        //to get the feature of the page
